...
def greedy_best_first(p):
    ...
    closeL = []
    chemin = []
    etat_actuel = Noeud(p.init, 0)
    openL.append(etat_actuel)

    while((not p.estBut(etat_actuel.etat)) and (openL != [])):
        closeL.append(etat_actuel)
        openL.remove(etat_actuel)

        successeur = etat_actuel.expand(p)
        for n in successeur:
            liste_etats = list()
        ...
        next_etat = openL[0]
        for n in openL[1::]:
            if(distManhattan(n.etat, p.but) < distManhattan(next_etat.etat, p.but)):
                next_etat = n
        etat_actuel = next_etat

    while(etat_actuel != None):
        chemin.append(etat_actuel.etat)
        etat_actuel = etat_actuel.pere
    print(chemin)
    return chemin[::-1]
...
def random_first(p):
    ...
    while(etat_actuel != None):
        ...
        etat_actuel = etat_actuel.pere
    print(chemin)
    print("Algo  : RandomFirst & temps de calcul:", time.time() - startTime)
    return chemin[::-1]
